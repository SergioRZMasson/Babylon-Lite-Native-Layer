...
// (the native layer built with -DBL_POLYFILL_XMLHTTPREQUEST=ON, the default), remote assets
// a scene references are downloaded over HTTP and cached locally, so test assets don't need
// to be present on disk up front. Without the polyfills these degrade gracefully
// (local-only): a remote asset that isn't on disk simply fails to load, exactly as before.
...
export function httpGetArrayBuffer(url: string): Promise<ArrayBuffer> {
...
export function ensureCached(url: string): Promise<void> {
    url = String(url);
    if (url.lastIndexOf("data:", 0) === 0 || !isRemoteUrl(url)) { return Promise.resolve(); }
    if (__bl_fileExists(url)) { return Promise.resolve(); }
    return httpGetArrayBuffer(url).then(function (ab) {
