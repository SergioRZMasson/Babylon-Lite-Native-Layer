// AUTO-GENERATED from Babylon-Lite lab/lite/src/lite/scene47.ts (esbuild, target es2017)
// Scene 47 — Physics Heightfield — tags: physics, gltf, procedural, std
// Run:  app.exe --prelude js/lite/index.js --script js/tests/scene47-physics-heightfield.js
...
const COLOR_BOX = [0.85, 0.27, 0.29];
const COLOR_SPHERE = [0.89, 0.71, 0.02];
const COLOR_CAPSULE = [0.34, 0.64, 0.65];
const COLOR_CYLINDER = [0.45, 0.55, 0.85];
const COLOR_HULL = [0.55, 0.78, 0.25];
const COLOR_MESH = [0.78, 0.36, 0.7];
const COLOR_GROUND = [0.52, 0.52, 0.52];
const BOX_PARAMS = { center: { x: 0, y: 0, z: 0 }, extents: { x: 2, y: 2, z: 2 } };
const SPHERE_PARAMS = { center: { x: 0, y: 0, z: 0 }, radius: 1 };
const CAPSULE_PARAMS = { pointA: { x: 0, y: -0.75, z: 0 }, pointB: { x: 0, y: 0.75, z: 0 }, radius: 0.75 };
const CYLINDER_PARAMS = { pointA: { x: 0, y: -1.5, z: 0 }, pointB: { x: 0, y: 1.5, z: 0 }, radius: 1 };
...
function findPrimaryMesh(container) {
  const meshes = [];
...
function createPrimitiveVisual(engine, type, color) {
  let mesh;
  switch (type) {
...
function rowXPositions(count, spacing) {
  const xs = [];
...
function createSeagullVisual(seagull, color) {
  const mesh = cloneTransformNode(seagull);
...
async function main() {
  ...
  ROW1.forEach((entry, i) => {
    if (entry.type === PhysicsShapeType.CONVEX_HULL || entry.type === PhysicsShapeType.MESH) {
      const mesh2 = createSeagullVisual(seagull, entry.color);
      mesh2.position.set(row1X[i], DROP_HEIGHT, ROW1_Z);
      addToScene(scene, mesh2);
      const shape = createPhysicsShape(world, { type: entry.type, mesh: mesh2 });
      const aggregate2 = createPhysicsAggregate(world, mesh2, entry.type, {
        mass: 1,
        friction: FRICTION,
        restitution: RESTITUTION,
        shape
      });
      showPhysicsBody(viewer, aggregate2.body);
      return;
    }
...
function bindDynamicBody(world, viewer, node, shape) {
