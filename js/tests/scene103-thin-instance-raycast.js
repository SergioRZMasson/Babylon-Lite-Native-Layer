// AUTO-GENERATED from Babylon-Lite lab/lite/src/lite/scene103.ts (esbuild, target es2017)
// Scene 103 — Thin-Instance Raycast Picking — tags: std, procedural, physics, thin-instance
// Run:  app.exe --prelude js/lite/index.js --script js/tests/scene103-thin-instance-raycast.js
...
function setTranslation(matrix, offset, x, y, z) {
  matrix[offset] = 1;
  matrix[offset + 5] = 1;
  matrix[offset + 10] = 1;
  matrix[offset + 12] = x;
...
function unprojectFar(invVp, ndcX, ndcY) {
  const x = ndcX;
  const y = ndcY;
  const z = 1;
...
async function main() {
  ...
  const seed = { value: 1063516 };
  const blockExtents = { x: BLOCK_SIZE, y: BLOCK_SIZE, z: BLOCK_SIZE };
  const bodyToInstance = /* @__PURE__ */ new Map();
  ...
  function castInto(target) {
    var _a;
    const result = physicsRaycast(world, camOrigin, target);
    const instance = result.hasHit && result.body ? (_a = bodyToInstance.get(result.body)) != null ? _a : -1 : -1;
  ...
  if (autoTest) {
    ...
    canvas.addEventListener("pointerdown", (evt) => {
      ...
      const dest = { x: camPos.x + dir.x * 1e3, y: camPos.y + dir.y * 1e3, z: camPos.z + dir.z * 1e3 };
