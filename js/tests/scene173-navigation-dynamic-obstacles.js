// AUTO-GENERATED from Babylon-Lite lab/lite/src/lite/scene173.ts (esbuild, target es2017)
// Scene 173 - Navigation Dynamic Obstacles — tags: navigation, recast, wasm, obstacles
// Run:  app.exe --prelude js/lite/index.js --script js/tests/scene173-navigation-dynamic-obstacles.js
...
async function main() {
  ...
  const cam = createArcRotateCamera(1.8, 1, 14, { x: 0, y: 0, z: 0 });
  scene.camera = cam;
  ...
  let boxObstacle = addBoxObstacle(nav, BOX_POS, BOX_HALF, BOX_ANGLE);
  ...
  function addEdgeTube(edgeStart, edgeEnd, position, rotationY) {
  ...
  function addBoxWireframe(half, position, rotationY) {
    const corners = [
  ...
  function addCylinderWireframe(height, radius, segments, position) {
    const h = height / 2;
  ...
  let boxTubes = addBoxWireframe(BOX_HALF, BOX_POS, BOX_ANGLE);
  ...
  onBeforeRender(scene, (deltaMs) => {
    ...
    if (!freeze) {
      updateNavMeshObstacles(nav);
      updateNavCrowd(crowd, deltaMs / 1e3);
