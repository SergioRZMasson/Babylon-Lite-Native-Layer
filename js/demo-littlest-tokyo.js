...
// uncompressed glTF geometry + skin, PBR materials lit by an environment cube (IBL),
// with the model's looping animation (cars, train, drifting smoke) playing on load.
//
// Differences from the web demo (browser-only bits the native layer doesn't need):
//   - no Draco/meshopt decoder bases (this LittlestTokyo.glb is uncompressed)
//   - the visible HDR skybox + on-screen progress/auto-rotate UI are omitted
//   - IBL uses the local environmentSpecular.env (the web demo used environment.dds)
//
